...
- (None currently — Substack and Money Stuff newsletters are routed to custom
  labels below)
...
2. **Is the sender from a Trusted Domain?** → KEEP (unless it's a Rule 3 automated
   notification)
3. **Is the user in the To/CC field and the email is part of an ongoing thread?**
   → KEEP
4. **Does the email require a response or action from the user?** → KEEP
5. **Does the email match any archive rule above?** → ARCHIVE
6. **When in doubt** → KEEP (false negatives are better than false positives)
